...
export async function POST(request: NextRequest) {
  ...
  let res: Response
  try {
    res = await fetch(`${process.env.BACKEND_URL}/api/v1/portfolios/capital`, {
      method: "POST",
      headers: {
  ...
  try {
    ...
    console.error(`[/api/portfolios/capital] Backend returned non-JSON ${res.status}:`, text.slice(0, 200))
    return NextResponse.json(
